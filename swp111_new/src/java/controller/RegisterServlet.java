...
public class RegisterServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
       ValidateRegister va=new ValidateRegister();
        ...
        boolean c=rd.checkEmail(email);
        User u=new User(username, rd.encode(password1), email, mobile, fullname);
        ...
        if (b == false) {
            ...
            request.setAttribute("checkemail", "Your email must starts with at least one alphanumeric character, can be followed by zero or more alphanumeric characters, must contain the at symbol (@), can have a dot (.) followed by one or more alphanumeric characters representing the TLD and in the range between 8 and 16 characters.");
            request.getRequestDispatcher("register.jsp").forward(request, response);
        }
        else {
...
//            rd.AddUser(username, password, email, mobile, fullname);
//            rd.AddRole();
//            request.setAttribute("d", "Register sucessfully");
//            request.getRequestDispatcher("register.jsp").forward(request, response);
